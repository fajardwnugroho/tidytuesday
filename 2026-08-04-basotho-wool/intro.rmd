This week we're exploring **Lesotho's Wool Trade**. The dataset comes from the [UN Comtrade Database](https://comtradeplus.un.org/) using the `comtradr` package. The country is known as "Lesotho", and its people are referred to as "Basotho" (pronounced "ba-soo-too").

Extract from the article "**The mountain men behind Lesotho’s wool wealth** - *More than 80,000 herders tend the country’s sheep and goats*" by Sechaba Mokhethi for [GroundUp News](https://groundup.org.za/article/the-mountain-men-behind-lesothos-wool-wealth/):
...
basotho_wool <- read.csv("basotho_wool.csv")
...
basotho_wool_plot <- basotho_wool_2023 |>
  ...
  adjust_size(width = 150, height = 60) |> 
  theme_tidyplot() +
  geom_text(
  ...
  theme(
    ...
    plot.title = element_text(family = "Plus Jakarta Sans", size = 20, face = "bold", margin = margin(b = 3)),
    #plot.title = element_text(family = "Plus Jakarta Sans", size = 18, face = "bold"),
    ...
    axis.text.y  = element_text(size = 10, margin = margin(r = 3)),
    #axis.text.y  = element_text(size = 8),
    axis.line.y = element_blank(),
    axis.ticks.y = element_blank(),
    #plot.caption = ggtext::element_markdown(size = 14, hjust = 0, margin = margin(t = 40), lineheight = 1.6),
    plot.caption = ggtext::element_markdown(size = 10, hjust = 0.3, margin = margin(t = 3), lineheight = 1.6),
    #plot.caption = ggtext::element_markdown(size = 8, hjust = 0.65, lineheight = 1.6),
